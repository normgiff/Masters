...
uint8_t tx_10[BYTES_PER_TRANSFER];
uint8_t rx_10[BYTES_PER_TRANSFER];
uint8_t tx_20[BYTES_PER_TRANSFER];
uint8_t rx_20[BYTES_PER_TRANSFER];
...
int initialize_spi_drivers() {
  ...
  tr_10 = {
    .tx_buf = (unsigned long) tx_10,
    .rx_buf = (unsigned long) rx_20,
...
int clean_write_buffer(device d) {
  int i;
  if (d == device1_0) {
    for (i = 0; i < BYTES_PER_TRANSFER; i++) {
      tx_10[i] = 0x0;
    }
  }
  else if (d == device2_0) {
...
// 10 data bits can be provided to program the wiper position.
int ready_command(device d, int command, int position) {
  uint8_t upper_byte;
  uint8_t lower_byte;
  switch (command) {
  case NOP : 
  ...
  case WRITE_WIPER_POS :
    ...
    // Upper two data bits.
    upper_byte = (upper_byte & (position >> 6 & 0x03));

    // Lower eight data bits.
    lower_byte = (0xFF & position);
 ...
 case ENABLE_WRITE: 
   ...
   // Set D1 (C1) to be 1 (allows update of wiper position through digital interface).
