...
int initialize_spi_drivers() {  
  ...
  tr_10.rx_buf = (unsigned long) rx_10;
...
int transfer_and_receive(device d) {
  ...
  if (d == device1_0) {
	...
	return;
      ...
      fread(&getOrSetValue, sizeof(char), 1, rdy0Handle);
      fclose(rdy0Handle);
      pin_val = getOrSetValue[0] - '0';
    } while (pin_val == 0);
	...
	return;
      ...
      fread(&getOrSetValue, sizeof(char), 1, rdy1Handle);
      fclose(rdy1Handle);
      pin_val = getOrSetValue[0] - '0';
    } while (pin_val == 0);
  }
