...
char UART_READ_BUF[BYTES_PER_TRANSFER];
char UART_WRITE_BUF[BYTES_PER_TRANSFER];
...
int send_template_vector(char* template_vector) {
  ...
  write(uart_fd, UART_WRITE_BUF, BYTES_PER_TRANSFER);

  // Wait for an acknowledgement.
  read(uart_fd, UART_READ_BUF, BYTES_PER_TRANSFER);
  ...
  memcpy(UART_WRITE_BUF, template_vector, BYTES_PER_TRANSFER);
...
int send_input_vector(char* input_vector) {
  ...
  memcpy(UART_WRITE_BUF, input_vector, BYTES_PER_TRANSFER);
...
// Returns 1 if the FPGA acknowledges all requests, 0 otherwise.
int send_cycle_vector(char* input_vector) {
  ...
  // Copy the input_vector data into the write buffer.
  memcpy(UART_WRITE_BUF, cycle_vector, BYTES_PER_TRANSFER);
...
int send_ff_vector(char* ff_vector) {
  ...
  memcpy(UART_WRITE_BUF, ff_vector, BYTES_PER_TRANSFER);
...
void retrieve_data(char* data) {
  ...
  memcpy(data, UART_READ_BUF, BYTES_PER_TRANSFER);
}
